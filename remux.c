...
//M7X0 BEGIN AK
ePesHeader AnalyzePesHeader(const uchar *Data, int Count, int &PesPayloadOffset, bool *ContinuationHeader, bool *Aligned)
{
  if (Count < 7)
  ...
  if ((Data[6] & 0xC0) == 0x80) { // MPEG 2
     ...
     if (Aligned) // Data Alignment Indicator set?
        *Aligned = (Data[6]&0x04) == 0x04; // If this flag this Flag is set 
                                           // Payload starts with ES-Startcode
                                           // (Audio-Sync-Word in Case of Audio,
                                           // Video depends on data stream alignment
                                           // descriptor in pmt, if not present 
                                           // Video is Slice or AU aligned.)
...
void cTS2PES::instant_repack(const uint8_t *Buf, int Count)
{
  ...
  while (c < Count && (mpeg == 0 || (mpeg == 1 && found < mpeg1_required) || (mpeg == 2 && found < 9)) && (found < 6 || !done)) { 
      // (found < 5 || !done)) { should be 6 to get the full length AK
        switch (found ) {
          case 0:
          case 1:
                  if (Buf[c] == 0x00)
          ...
          case 6:
                //  if (!done) {  
                // this is always true, see loop condition AK
                     flag1 = Buf[c++];
                     found++;
          ...
          case 7:
                 ...
                 // this is alway true, see loop condition AK
                     flag2 = Buf[c++];
                     found++;
                 //    }
                  break;
  ...
  if (done || ((mpeg == 2 && found >= 9) || (mpeg == 1 && found >= mpeg1_required))) {
     switch (cid) {
       ...
       case PRIVATE_STREAM1:
            ...
            if (mpeg == 2 && found == 9 && count < 9) { // Should not be copied twice (rare but possible) AK
               write_ipack(&flag1, 1);
               write_ipack(&flag2, 1);
               write_ipack(&hlength, 1);
               }

            if (mpeg == 1 && found == mpeg1_required && count < mpeg1_required) { //Should not be copied twice (rare but possible) AK
               write_ipack(&flag1, 1);
               if (mpeg1_required > 7) {
                  write_ipack(&flag2, 1);
                  write_ipack(&hlength, 1);
...
/*
On which reason this is done explicily in case of PTS present only in Header?
This gets copied next anyway. (In error case, if plength + 6 is smaller 
than hlength + 9 ,the packet is broken in both ways) AK
...
// This can happen only once at a time AK
              if (c < Count && found < plength + 6) {
                  int l = Count - c;
...
void cTS2PES::ts_to_pes(const uint8_t *Buf) // don't need count (=188)
{
  ...
  if (((Buf[3] ^ ccCounter) & CONT_CNT_MASK) || ccCounter == -1) {
...
//Lets see what is going on here
  resultBuffer = new cRingBufferLinear(RESULTBUFFERSIZE, IPACKS, true, "Result");
//M7X0 END AK
